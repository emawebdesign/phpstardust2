...
class PsdComponent extends Component {
	...
	public function getJwt($fields = array()) {
		...
		$signature = base64_encode(hash_hmac('sha256', $header_payload, Configure::read('Psd.secretkey'), true));
	...
	public function checkJwt($token = NULL) {
		...
		$resultedsignature = base64_encode(hash_hmac('sha256', $recievedHeaderAndPayload, Configure::read('Psd.secretkey'), true));
